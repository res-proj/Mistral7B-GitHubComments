...
if __name__ == "__main__":
    ...
    data_file=base_path + 'cross_version_teams_strong_comment_only.csv'
    ...
    df['Theme'] = pd.Series(dtype='str')

    print('start labelling')
    for idx, row in df.iterrows():
        df.loc[idx, 'Theme'] = get_theme(ft_model, idx, row['comment'])

    print('save data')
    df.to_csv(base_path + f'/cross_version_team_comments{apdx}.csv', mode='w')
